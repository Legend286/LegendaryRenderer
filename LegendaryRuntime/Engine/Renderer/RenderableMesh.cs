...
namespace Geometry
{
    public class RenderableMesh : GameObject
    {
        ...
        public static int LoadTexture(string path, string modelRootPath = "", bool HDR = false)
        {
            string actualPath = path;
            actualPath = actualPath.Split('\\').Last();
            actualPath = actualPath.Split('/').Last();
            ...
            {
                finalPath = Directory.GetParent(Directory.GetParent(modelRootPath).FullName).FullName + "/textures/" + actualPath;
            }

            string uniqueKey = Path.GetFullPath(finalPath);

            Console.WriteLine($"Attempting to Load Texture {uniqueKey}...");
            if (File.Exists(finalPath) && !LoadedTextures.ContainsKey(uniqueKey))
            {
                if (!HDR)
                {
                    var image = Image.Load<Rgba32>(finalPath); // Load the image as RGBA
                ...
                {
                    var image = Image.Load<Rgb48>(finalPath); // Load the image as RGBA
